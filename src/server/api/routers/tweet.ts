import { protectedProcedure, publicProcedure, type createTRPCContext } from './../trpc';
import { z } from "zod";
import {
  createTRPCRouter,
...
export const tweetRouter = createTRPCRouter({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input: { content }, ctx }) => {
      const tweet = ctx.prisma.tweet.create({ data: { content, userId: ctx.session.user.id } })
...
async function getInfiniteTweets({
  ...
  const currentUserId: string | undefined = ctx.session?.user?.id;
